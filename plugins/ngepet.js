...
const moneyprem = 2000000

let handler = async (m, { isPrems }) => {
    let time = global.db.data.users[m.sender].lastweekly + 604800000
  if (new Date - global.db.data.users[m.sender].lastweekly < 604800000) throw `Anda sudah ngepet🐗, sembunyi dulu agar tidak ketawan\nsembunyi selama ${msToTime(time - new Date())} lagi`
    //    conn.reply(m.chat, `Anda baru saja ngepet🐗 dan mendapatkan :`, m)
