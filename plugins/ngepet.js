...
let handler = async (m, { conn, usedPrefix, text }) => {
  ...
  if (new Date - global.db.data.users[m.sender].lastnambang< 28800000) throw `Anda sudah ngepet\nMohon sembunyi terlebih dahulu\nsembunyi selama ${msToTime(time - new Date())} lagi`
 ...
 // else conn.reply(m.chat, 'Minimal 10 Aqua untuk kamu menambang, karena menambang membutuhkan energi, seperti mendapat dia >,,<', m)
  ...
  setTimeout(() => {
					conn.reply(m.chat, `Ngepet lagi yuk kak 🐗`, m)
					}, timeout)
}
handler.help = ['ngepet']
handler.tags = ['rpg']
